...
const bundledPlugins: BundledModules = {};
const bundledPresets: BundledModules = {};
 ...
 */
function loadBuiltin(bundled: BundledModules, name: BabelPluginOrPresetName) {
  if (isArray(name) && typeof name[0] === 'string') {
    if (bundled.hasOwnProperty(name[0])) {
      return [bundled[name[0]]].concat(name.slice(1));
    }
    return;
...
export function loadPlugin(name: BabelPluginOrPresetName) {
  return loadBuiltin(bundledPlugins, name);
}
...
export function processOptions(options: any = {}) {
  const presets = (options.presets || []).map(presetName => {
    const preset = loadBuiltin(bundledPresets, presetName);
  ...
  const plugins = (options.plugins || []).map(pluginName => {
    const plugin = loadBuiltin(bundledPlugins, pluginName);
...
export function registerPlugin(name, plugin) {
  if (bundledPlugins.hasOwnProperty(name)) {
    console.warn(
      `A plugin named "${name}" is already registered, it will be overridden`
    );
  }
  bundledPlugins[name] = plugin;
}
...
export function registerPreset(name, preset) {
  if (bundledPresets.hasOwnProperty(name)) {
    console.warn(
      `A preset named "${name}" is already registered, it will be overridden`
    );
  }
  bundledPresets[name] = preset;
}
